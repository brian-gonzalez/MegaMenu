...
import {whichTransition} from '@borngroup/born-utilities';
...
export default class Megamenu{
    constructor(options) {
        ...
        let triggers = this.menu.querySelectorAll(this.options.triggerSelector);
    ...
    /**
     * Sets up the properties for the menu.
     * These can be set on a breakpoint basis.
     *
     * Properties::
        events: [String] string with events to listen to.
        menuActiveClass: [String] class to add to the 'menu' when the dropdowns are active
        itemActiveClass: [String] class to add to the 'item' when it is active
        unsetOnMouseleave: [Boolean] Determines if 'menu' should be closed when hovering out of it.
        disableUnsetSelf: [String | Boolean] Determines if item should close itself when actioning on it (i.e. clicks).
                                            Pass a class name to disallow the referenced element from closing.
        responsive: [Object] Sets breakpoints settings using this format:
                    responsive: [
                        {
        ...
        clickThroughSelector: [String] On an anchor, specify a class which will enable clicks on if said anchor is also a 'menu' 'item'. Flag to false to disable link clicking.
        hoverDelay: [Integer Milliseconds] Enables a delay on activating the menu.
     ...
     * @param  {[Node]} trigger [Node where we're getting the parent Node from]
     * @return {[Node]} [Parent of the passed element]
     */
    getParent(trigger) {
     ...
     * @param  {[Node]} trigger [Node where we're getting the target Node from]
     * @return {[Node]} [Target of the passed element]
     */
    getTarget(trigger) {
     ...
     */
    toggleItemActive(trigger, isMousehover) {
    ...
    setItemActive(trigger) {
        ...
        if ( !this.menu.isActive ) {
            ...
            if ( canDetectTransition ) {
                activeWaitEl.addEventListener(whichTransition(), transitionEndHandler);
            } else {
                this.setMenuActive();
            }
        }
        ...
        function transitionEndHandler(evt) {
            if (SCOPE.options.waitForTransition.property === evt.propertyName || SCOPE.options.waitForTransition.property === 'all') {
                ...
                this.removeEventListener(whichTransition(), transitionEndHandler);
            }
        }
    }
    ...
    /**
     * Create a list of the items that are being activated.
     * @param {[type]} trigger [description]
     */
    setLastItemActive(trigger) {
        this._activeItem = this._activeItem || [];

        this._activeItem.push(trigger);
    }

    removeLastItemActive() {
        if (this._activeItem) {
    ...
    /**
     * Unsets all of the [trigger] relatives, then '_afterMenuUnset' is fired
     */
    unsetRelatives(trigger, callback) {
